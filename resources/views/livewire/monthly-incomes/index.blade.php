<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
            <flux:heading size="xl">@lang('messages.monthly_incomes.title')</flux:heading>
            <flux:text>@lang('messages.monthly_incomes.subtitle')</flux:text>
        </div>
    </div>
    ...
    <div class="grid gap-3 md:grid-cols-1">
        <flux:input wire:model.live.debounce.300ms="search" :label="__('messages.common.search')" :placeholder="__('messages.monthly_incomes.search_placeholder')" />
    </div>

    <div class="rounded-xl border border-zinc-200 p-4 dark:border-zinc-700">
        <div class="mb-4 flex items-center justify-between gap-3">
            <flux:heading size="sm">
                {{ $editingItemId ? __('messages.monthly_incomes.edit_title') : __('messages.monthly_incomes.new_title') }}
            </flux:heading>
            @if ($editingItemId)
        ...
        @if ($billingPeriods->isEmpty())
            <flux:callout variant="warning" icon="exclamation-triangle">
                @lang('messages.monthly_incomes.requirements')
            </flux:callout>
        @else
            <form wire:submit="saveItem" class="grid gap-3 md:grid-cols-2">
                ...
                <flux:input wire:model="name" :label="__('messages.common.title')" required />
                <flux:input wire:model="price" :label="__('messages.common.amount')" type="number" step="0.01" min="0.01" required />
                <div class="md:col-span-2">
                    <flux:textarea wire:model="description" :label="__('messages.form.note')" rows="3" />
                </div>
                <div class="md:col-span-2">
    ...
    <x-ui.table>
        <x-ui.table.head>
            <tr>
                ...
                <x-ui.table.th align="right">@lang('messages.monthly_incomes.amount_for_period')</x-ui.table.th>
        ...
        <x-ui.table.body>
            @forelse ($items as $item)
                <x-ui.table.row wire:key="monthly-income-item-{{ $item->id }}">
                    <x-ui.table.td class="font-medium">{{ $item->name }}</x-ui.table.td>
                    <x-ui.table.td>{{ $item->billingPeriod?->name }}</x-ui.table.td>
                    <x-ui.table.td>{{ $item->description ?: '-' }}</x-ui.table.td>
                    <x-ui.table.td align="right">{{ number_format((float) $item->price, 2, ',', '.') }} RSD</x-ui.table.td>
                    <x-ui.table.td align="right">
                        @if ($item->billingPeriod?->key === 'yearly')
                            {{ number_format((float) $item->price / 12, 2, ',', '.') }} RSD
                        @else
                            {{ number_format((float) $item->price, 2, ',', '.') }} RSD
                        @endif
                    </x-ui.table.td>
                    <x-ui.table.td align="right">
                        <x-ui.table.actions>
            ...
            @empty
                <x-ui.table.empty colspan="6">
                    @lang('messages.monthly_incomes.empty')
                </x-ui.table.empty>
            @endforelse
        </x-ui.table.body>
    </x-ui.table>
    ...
    <div class="flex items-center justify-end rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-semibold dark:border-zinc-700 dark:bg-zinc-900">
        <span>@lang('messages.monthly_incomes.total'): {{ number_format((float) $monthlyTotal, 2, ',', '.') }} RSD</span>
    </div>
